...
After obtaining the dataset and beginning to work with it, we did not move forward with RQ2. **[Either a) insert more information about why specifically you chose to bring up the research question even though you dropped it or b) consider leaving it out in the intro/adding it to the discussion. - AG]** In addition, we decided to explore the relationship between internalizing and externalizing behaviors at age 9 and the number of suspensions/expulsions at age 15. 
...
**[Great plot! It may be a bit distracting to have the missing data included in this plot; perhaps remove it? Also, do you have any information on how "other" is defined? Again, if not meaningful, perhaps you can exclude from this graph or collapse with another category. These choices might make the graph easier to read. -AG]**
...
**[Plots 2, 3, and 5 have an x-scale that was partially removed by your regular expression ("never", "or more times", "or times"). Consider just removing the first two characters instead of removing all digits using the substring() function. - AG]**
...
mod_db_ext_15 %>%
    mutate(intercept = map_dfr(model, pull_coef)) # I wonder if you can go ahead and pipe these into your code above as to not make this chunk repetitive
